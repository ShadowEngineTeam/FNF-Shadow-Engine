...
class ShadowStyle {
    ...
    public static function lerpColor(from:FlxColor, to:FlxColor, t:Float):FlxColor {
        ...
        return FlxColor.fromRGBFloat(
            from.redFloat + (to.redFloat - from.redFloat) * t,
            from.greenFloat + (to.greenFloat - from.greenFloat) * t,
            from.blueFloat + (to.blueFloat - from.blueFloat) * t,
            from.alphaFloat + (to.alphaFloat - from.alphaFloat) * t
        );
    }

    public static function brighten(color:FlxColor, amount:Float):FlxColor {
        return FlxColor.fromRGBFloat(
            Math.min(1, color.redFloat + amount),
            Math.min(1, color.greenFloat + amount),
            Math.min(1, color.blueFloat + amount),
            color.alphaFloat
        );
    }
}
