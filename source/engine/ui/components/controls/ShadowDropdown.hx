...
class ShadowDropdown extends FlxSpriteGroup {
	...
	override public function destroy():Void {
		if (isOpen) closeList();
		if (_clickConsumer == this) _clickConsumer = null;
	...
	function applyCamerasToChildren():Void {
		if (header != null) header.cameras = cameras;
		if (headerText != null) headerText.cameras = cameras;
		if (arrow != null) arrow.cameras = cameras;
		if (dropList != null) dropList.cameras = cameras;

		if (listBg != null) listBg.cameras = cameras;
		if (_rowHighlight != null) _rowHighlight.cameras = cameras;
		for (t in _rowItems) if (t != null) t.cameras = cameras;
	}

	public static function closeAllOpen():Void {
		for (dropdown in _instances) {
			if (dropdown != null && dropdown.isOpen) dropdown.closeList();
		}
	}
	...
	public static function isClickCaptured():Bool {
		if (_clickConsumer == null) return false;
		if (_clickConsumedFrame != Std.int(FlxG.game.ticks)) return false;
	...
	private static inline function clearClickConsumer(dropdown:ShadowDropdown):Void {
		if (_clickConsumer == dropdown) _clickConsumer = null;
	}
	...
	inline function mouseOverSpriteScreenRect(s:FlxSprite, cam:FlxCamera):Bool {
		...
		return _tmpMouse.x >= _tmpBg.x && _tmpMouse.x < _tmpBg.x + s.width
			&& _tmpMouse.y >= _tmpBg.y && _tmpMouse.y < _tmpBg.y + s.height;
	}
	...
	inline function getHoveredIndexOnList(cam:FlxCamera):Int {
		if (!isMouseOverList(cam)) return -1;
	...
	function ensureSelectedVisible():Void {
		if (options == null || options.length == 0) { _scrollIndex = 0; return; }

		var visibleCount:Int = Std.int(Math.min(options.length, _maxVisible));
		if (visibleCount <= 0) { _scrollIndex = 0; return; }

		var maxScroll:Int = Std.int(Math.max(0, options.length - visibleCount));
		if (_selectedIndex < _scrollIndex) _scrollIndex = _selectedIndex;
		else if (_selectedIndex >= _scrollIndex + visibleCount) _scrollIndex = _selectedIndex - visibleCount + 1;
	...
	function buildDropList():Void {
		var visibleCount:Int = Std.int(Math.min(options.length, _maxVisible));
		if (visibleCount <= 0) {
			if (dropList != null) { dropList.visible = false; dropList.exists = false; dropList.active = false; }
			if (listBg != null) listBg.visible = false;
			if (_rowHighlight != null) _rowHighlight.visible = false;
			for (row in _rowItems) if (row != null) row.visible = false;
			return;
		}
		...
		for (i in 0..._width) listBg.pixels.setPixel32(i, listHeight - 1, ShadowStyle.BORDER_DARK);
		...
		for (i in 0..._maxVisible) {
			...
			if (i < visibleCount) {
				...
				if (optionIndex >= options.length) { row.visible = false; continue; }
	...
	inline function closeList():Void {
		...
		if (!_headerHovered) drawHeader(ShadowStyle.BORDER_DARK);
	}

	function get_selectedIndex():Int return _selectedIndex;
	...
	function set_selectedIndex(value:Int):Int {
		...
		if (headerText != null) headerText.text = options[_selectedIndex];

		ensureSelectedVisible();
		if (isOpen) buildDropList();

		return _selectedIndex;
	}

	function get_selectedLabel():String {
		if (options != null && options.length > _selectedIndex && _selectedIndex >= 0) return options[_selectedIndex];
		return "";
	}
	...
	override function update(elapsed:Float) {
		...
		if (overHeader && !_headerHovered) {
			...
			if (!isOpen) drawHeader(ShadowStyle.BORDER_DARK);
		}
		...
		if (!_ignoreUntilMouseRelease && anyClick && nowTick > _ignoreClickUntilTick) {
			...
			if (isOpen && leftClick) {
				...
				if (idx != -1) {
					selectedIndex = idx;
					if (callback != null) callback(_selectedIndex);
					closeList();
					consumeClick(this);
					return;
				}
...
class ShadowDropdownList extends FlxSpriteGroup {
	...
	override function set_visible(Value:Bool):Bool {
		if (Value && !canToggle()) return visible;
		return super.set_visible(Value);
	}

	override function set_exists(Value:Bool):Bool {
		if (Value && !canToggle()) return exists;
		return super.set_exists(Value);
	}

	override function set_active(Value:Bool):Bool {
		if (Value && !canToggle()) return active;
		return super.set_active(Value);
	}
